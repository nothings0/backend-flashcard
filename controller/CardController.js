...
const CardController = {
    createCard: async(req, res, next) => {
        try {
            const { term } = req.body
            const { title, description, share, background } = req.body
            const newCard = new Card({title, description, share, background, user: req.user._id})
            await newCard.save()
            let termCount = await Term.find({ cardId: newCard._id }).count()
            let terms = []
            for(let i = 0; i < term.length; i++){
                const termItem = {
                    prompt: term[i].prompt, answer: term[i].answer, cardId: newCard._id,
    ...
    getAllCard: async(req, res, next) => {
        ...
        try {
            if(query){
                if(query === 'trend'){
                    cards = await Card.find({share: true}).populate('user', 'username').sort({views: -1}).
                    limit(limit)
                    ...
                    cards = await Card.find({share: true}).populate('user', 'username').sort({ "rate.total": 1, "rate.quantity": 1 }).limit(limit)
                    const cnt = await Card.find({share: true}).count()
                    count = cnt > 10 ? 10 : cnt
                    title = 'Đánh giá cao'
                }
                    ...
                    const {userId} = req.query
                    cards = await Card.find({user: userId}).populate("user", "username -_id").
            limit(limit)
    ...
    getHome: async(req, res, next) => {
        ...
        const {userId} = req.query
        try {
            const populateCards = await Card.find({share: true}).populate('user', 'username').sort({views: -1}).
            limit(limit)
            
            const rateCards = await Card.find({share: true}).populate('user', 'username').sort({ "rate.total": -1, "rate.quantity": -1 }).limit(limit)
    ...
    getCardInUser: async(req, res, next) => {
        ...
        const { limit } = Pagination(req)
        try {
            const cardsOfUser = await Card.find({user: userId}).populate("user", "username -_id").
            limit(limit)
            const cardSaved = await CardSaved.find({user: userId}).
            limit(limit)
            ...
            res.status(200).json({
                library: {
                    cards: cardsOfUser, total: total1, title: "Học phần của bạn"
                },
                cardSaved: {
                    cards: cardsSaved2, total: total2, title: "Học phần đã lưu"
                }
            })
        } catch (err) {
            next(err)
    ...
    getCardById: async(req, res, next) => {
        ...
        try {
            const cards = await Card.findOne({ _id: cardId }).populate('user', 'username')
            const terms = await Term.find({ cardId }).sort({position: 1}).skip(skip).limit(limit)
            ...
            if (card.user.valueOf() === req.body.userId){
                try {
                    ...
                    for(let i = 0; i < termArr.length; i++){
                        if(terms[i]){
                            if(terms[i].prompt !== termArr[i].prompt || terms[i].answer !== termArr[i].answer){
                            ...
                            const newTerm = new Term({
                                prompt: termArr[i].prompt, answer: termArr[i].answer, cardId: cardId,
                                position: termCount > 0 ? termCount : 0
                            })
                            await newTerm.save()
                            termCount++
    ...
    savedCard: async(req, res, next) => {
        ...
        try {
            ...
            if(cardSaved) return res.status(200).json({code: 200, msg: "Đã được lưu trước đó"})
            const card = await Card.findById(cardId)
            if(card.user.valueOf() === userId) return res.status(400).json({code: 400, msg: "Bạn chỉ được lưu học phần người khác"})
    ...
    rateCard: async (req, res, next) => {
        ...
        try {
            ...
            if(!rate){
                ...
                await newRate.save()
                await Card.findByIdAndUpdate(cardId, {$inc: {"rate.total": 1, "rate.quantity": rateNum}})
            }else{
                let inc = rateNum - rate.rate
                await rate.updateOne({ $set: {rate: rateNum} })
                await Card.findByIdAndUpdate(cardId, {$inc: {"rate.quantity": inc}})
            }
    ...
    createCardAdmin: async (req, res, next) => {
        try {
            ...
            const userId = req.user._id
            const newCard = new Card({title, description: des, user:userId, background})
            await newCard.save()
            let termCount = await Term.find({ cardId: newCard._id }).count()
            let terms = []
            for(let i = 0; i < term.length; i++){
