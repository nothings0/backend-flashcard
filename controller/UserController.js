...
const CLIENT_URL = "https://x-fluxquiz.netlify.app"
...
const UserController = {
    register: async(req, res, next) => {
        try {
            ...
            if(userCheck) return res.status(400).json({msg: 'Tài khoản đã tồn tại'})
            ...
            return res.status(200).json({ msg: "Vui lòng kiểm tra email để hoàn thành đăng ký tài khoản" })
        } catch (err) {
            next(err)
        }
    },
    activeAccount: async(req, res, next) => {
        try {
          ...
          if(!newUser) return res.status(400).json({msg: "Invalid authentication.", code: 400})
    ...
    generateAccessToken: (user) => {
        return jwt.sign({_id: user._id, isAdmin: user.isAdmin}, process.env.SECRET_KEY, {expiresIn: '1d'})
    },
    generateRefreshToken: (user) => {
        return jwt.sign({_id: user._id, isAdmin: user.isAdmin}, process.env.REFRESH_KEY, {expiresIn: '7d'})
    },
    login: async(req, res, next) => {
        ...
        try {
            ...
            if(!user){
                return res.status(400).json({msg: "Tài khoản không tồn tại", code: 400})
            }
    ...
    logout: async(req, res, next) => {
        try {
            ...
            if(!refreshToken) return res.status(401).json({msg: "You're not authenticated", code: 401})
            const decodeDToken = jwt_decode(refreshToken)
            let date = new Date()
            if(decodeDToken.exp < date.getTime() / 1000) return res.status(401).json({code: 401, msg: "token expired"})
    ...
    reqRefreshToken: async(req, res, next) => {
        try {
            const refreshToken = req.cookies.refreshToken
            if(!refreshToken) return res.status(401).json({code: 401, msg: "You're not authenticated"})
            const decodeDToken = jwt_decode(refreshToken)
            let date = new Date()
            if(decodeDToken.exp < date.getTime() / 1000) return res.status(401).json({code: 401, msg: "token expired"})
            jwt.verify(refreshToken, process.env.REFRESH_KEY, (err, user) => {
                ...
                client.set(user._id.toString(), newRefreshToken, 'EX', 7 * 24 * 60 * 60, (err, reply) => {
                    if(err) return next(err)
                })
    ...
    getUser: async(req, res, next) => {
        ...
        try {
            const user = await User.findOne({username}, { isAdmin: 0, password: 0 });
            const cards = await Card.find({user: user._id}).limit(4)
            const achieve = await Achieve.findOne({user: user._id}, {user: 0, _id: 0})
            res.status(200).json({user, cards, achieve })
        } catch (error) {
            next(error)
        }
    },
    ...
    updateAva: async (req, res, next) => {
        ...
        // }
        try {
            cloudinary.v2.uploader.upload(file.tempFilePath, {folder: "ava-user"}, async(err, result)=>{
                if(err) throw err;
                // removeTmp(file.tempFilePath)
                const img = result.secure_url
                const data = await User.findByIdAndUpdate(userId, {profilePic: img}, {new: true})
    ...
    forgotPassword: async (req, res, next) => {
        ...
        try {
            ...
            return res.status(200).json({ msg: "Vui lòng kiểm tra email của bạn để tiến hành lấy lại mật khẩu" })
        } catch (error) {
            next(error)
        }
    },
    ...
    contactService: async (req, res, next) => {
        ...
        try {
            ...
            await newData.save()
            res.status(201).json({msg: "Gửi thành công, chúng tôi sẽ trả lời nhanh nhất", code: 201})
        } catch (err) {
            next(err)
        }
    },
    loginGoogle: async (req, res, next) => {
        try {
            ...
            const verify = await OAuthClient.verifyIdToken({idToken: id_token, audience: `${process.env.CLIENT_ID}`})
            const {email, email_verified, name, picture} = verify.getPayload()
            
            if(!email_verified)
                return res.status(401).json({msg: "Email chưa hoàn tất xác thực Google"})
            ...
            if(user){
                ...
                await newUser.save()
                const notifi = new Notification({content: "Chào mừng bạn đến với fluxquiz.com", user: newUser._id})
                await notifi.save()
...
const loginUser = async (user, password, res) => {
    ...
    if(!validPassword){
        return res.status(400).json({msg: "Tài khoản hoặc mật khẩu không chính xác", code: 400})
    }
    if(user && validPassword){
        const accessToken = UserController.generateAccessToken(user)
        const refreshToken = UserController.generateRefreshToken(user)
        client.set(user._id.toString(), refreshToken, 'EX', 7 * 24 * 60 * 60, (err, reply) => {
            if(err) next(err)
        })
        ...
        const userPL = await User.findOne({username: user.username}, {password: 0})
...
const registerUser = async (newUser, res) => {
    ...
    await user.save()
    const notifi = new Notification({content: "Chào mừng bạn đến với FLUXQUIZ.COM", user: user._id})
    await notifi.save()
