...
const AIController = {
  getAIStream: async (req, res) => {
    ...
    // Kiểm tra context
    if (!Array.isArray(context) || context.some(msg => !msg.sender || !msg.message)) {
      return res.status(400).json({ msg: 'Hihi, context không đúng định dạng nè! 😿' });
    }
    ...
    try {
      ...
      const latestCard = await Card.findOne({ user: userId }).sort({ createdAt: -1 });
      ...
      const isTranslationQuestion = userMessage.match(/dịch.*(sang|tới|qua)\s*(tiếng\s*\w+)/i);
      ...
      while (retries < maxRetries) {
        try {
          const intentResult = await intentChatSession.sendMessage(intentPrompt);
      ...
      if (intentData.intent === 'create_flashcard' && intentData.languagePair && intentData.topic) {
        ...
        const card = new Card({
          ...
          slug: slugify(cardContent.title + '-' + lastThreeChars, { lower: true, strict: true }),
          user: userId,
          share: true,
          views: 5,
        ...
        if (intentData.quantity) {
          const updatePrompt = `
            ...
            Hãy tạo thêm ${intentData.quantity} từ vựng mới cho bộ flashcard hiện có.
          ...
          while (retries < maxRetries) {
            try {
              const updateResult = await intentChatSession.sendMessage(updatePrompt);
              updateContent = JSON.parse(updateResult.response.text());
              break;
            } catch (parseError) {
              retries++;
              if (retries === maxRetries) {
                res.write('Hihi, tớ gặp khó khăn khi thêm từ mới rồi! 😿 Hãy thử lại nha!');
                res.end();
                return;
              }
              await new Promise(resolve => setTimeout(resolve, 500));
          ...
          const lastPosition = await Term.find({ cardId: card._id }).sort({ position: -1 }).limit(1);
          const startPosition = lastPosition.length > 0 ? lastPosition[0].position + 1 : 1;
        ...
        if (intentData.newDescription) updateFields.description = intentData.newDescription;
        if (intentData.newBackground) updateFields.background = intentData.newBackground;
        if (intentData.isPrivate !== null) updateFields.share = !intentData.isPrivate;
        if (intentData.password) updateFields.password = intentData.password;
        if (intentData.newTitle)
          updateFields.slug = slugify(intentData.newTitle, { lower: true, strict: true });

        if (Object.keys(updateFields).length > 0) {
          await Card.updateOne({ _id: card._id }, { $set: updateFields });
        }

        const cardLink = `https://fluxquiz.vercel.app/card/${updateFields.slug || card.slug}`;
